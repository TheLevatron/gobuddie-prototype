...
function buildMonthDays(ym) {
  const [y,m] = ym.split('-').map(Number); const last = new Date(y,m,0); const days=[];
...
// Chat
const chatLog = el('chat-log'); const chatForm = el('chat-form'); const chatInput = el('chat-input');
function addMsg(role, text) {
  if (!chatLog) return;
  const row = document.createElement('div'); row.className = `msg ${role}`;
  const bubble = document.createElement('div'); bubble.className='bubble'; bubble.textContent=text;
  const meta = document.createElement('div'); meta.className='meta'; meta.textContent = role==='user'?'You':'System';
  row.appendChild(meta); row.appendChild(bubble); chatLog.appendChild(row); chatLog.scrollTop = chatLog.scrollHeight;
}
document.querySelectorAll('.quick-actions .chip').forEach(b => {
  b.addEventListener('click', () => { if(!chatForm||!chatInput) return; chatInput.value=b.dataset.msg; chatForm.dispatchEvent(new Event('submit',{cancelable:true})); });
});
if (chatForm) {
  chatForm.addEventListener('submit', (e) => {
    e.preventDefault();
    const text = (chatInput?.value || '').trim(); if(!text) return;
    addMsg('user', text); processChat(text); chatInput.value='';
  });
}
...
function processChat(text) {
  ...
  if (m){ const bill=createBill(m[1], Number(m[2]), m[3], false); addMsg('system', `Added bill: ${bill.name} ₱${bill.amount.toFixed(0)} due ${bill.due}.`); renderAll(); return; }

  m = text.match(/^schedule\s+payment\s+(.+?)\s+(\d+(?:\.\d+)?)\s+on\s+(\d{4}-\d{2}-\d{2})$/i);
  if (m){ const [_, name, amountStr, date]=m; const amount=Number(amountStr); const bill=findBill(name,amount,date);
    if (bill){ bill.scheduled=true; addMsg('system', `Scheduled ${bill.name} on ${date}.`); } else { createBill(name, amount, date, true); addMsg('system', `Created & scheduled ${name} ₱${amount} for ${date}.`); }
    renderAll(); return; }

  m = text.match(/^prioritize\s+bills\s+if\s+monthly\s+budget\s+is\s+(\d+(?:\.\d+)?)$/i);
  if (m){ store.monthlyBudget=Number(m[1]); prioritizeByBudget(); addMsg('system', `Prioritized bills for monthly budget ₱${store.monthlyBudget}.`); renderAll(); return; }

  if (/^show\s+reminders$/i.test(text)) {
    const summary = store.reminders.map(r => { const bill = store.bills.find(b=>b.id===r.billId); return `${bill?.name ?? 'Bill'} on ${r.when}`; });
    addMsg('system', summary.length ? `Reminders:\n- ${summary.join('\n- ')}` : 'No reminders yet.'); renderAll(); return;
  }

  if (/^renew\s+otp$/i.test(text)) { store.otpMonth=currentMonthStr(); addMsg('system', `OTP renewed for ${store.otpMonth}.`); const status = document.getElementById('otp-status'); if(status) status.textContent=`Active ${store.otpMonth}`; return; }

  addMsg('system','Try:\n- Add Meralco 1500 due YYYY-MM-DD\n- Schedule payment Meralco 1500 on YYYY-MM-DD\n- Prioritize bills if monthly budget is 3000\n- Show reminders\n- Renew OTP');
}

// Calendar
const calendarGrid = el('calendar-grid'); const calendarMonth = el('calendar-month'); const monthlyBudgetInput = el('monthly-budget'); const recalcBtn = el('recalculate-priority');
if (calendarMonth) calendarMonth.value = currentMonthStr();
if (recalcBtn) recalcBtn.addEventListener('click', () => { store.monthlyBudget = Number(monthlyBudgetInput?.value || 0); prioritizeByBudget(); renderAll(); });
...
function renderCalendar() {
  ...
  buildMonthDays(ym).forEach(day => {
    ...
    const dateLabel = document.createElement('div'); dateLabel.className='date'; dateLabel.textContent=day.dateStr; cell.appendChild(dateLabel);
    store.bills.filter(b => b.due === day.dateStr).forEach(b => {
      const tag = document.createElement('div'); tag.className='bill';
      if (b.scheduled) tag.classList.add('scheduled'); if (b.prioritized) tag.classList.add('prioritized'); if (!b.prioritized && store.monthlyBudget>0) tag.classList.add('overflow');
      tag.textContent = `${b.name} ₱${b.amount}`; cell.appendChild(tag);
    });
...
function prioritizeByBudget() {
  ...
  const sorted = [...monthBills].sort((a,b) => (b.scheduled-a.scheduled) || (a.amount-b.amount) || (new Date(a.due)-new Date(b.due)));
  let acc=0; sorted.forEach(b => { if (acc + b.amount <= store.monthlyBudget){ b.prioritized=true; acc+=b.amount; } else { b.prioritized=false; } });
}
...
function renderDashboard() {
  ...
  const totalBillsEl = el('stat-total-bills'); const scheduledEl = el('stat-scheduled'); const remainingEl = el('stat-remaining');
  if (totalBillsEl) totalBillsEl.textContent = bills.length;
  ...
  const onTimeEl = el('stat-on-time'); const trustEl = el('stat-trust'); const trustBar = document.getElementById('trust-bar');
  if (onTimeEl) onTimeEl.textContent = `${onTimeRate}%`;
  if (trustEl) trustEl.textContent = trustScore;
  if (trustBar) trustBar.style.width = `${onTimeRate}%`;

  const pointsEl = el('stat-points'); if (pointsEl) pointsEl.textContent = store.points;
  ...
  if (remindersUl) {
    ...
    store.reminders.filter(r => r.when.startsWith(ym)).forEach(r => {
      ...
      const li = document.createElement('li'); li.className='list-item'; li.textContent = `${b?.name ?? 'Bill'} — ${r.when}`; remindersUl.appendChild(li);
    });
  }
...
if (calcBudgetBtn) {
  calcBudgetBtn.addEventListener('click', (e) => {
    e.preventDefault();
    const income = Number((el('income')?.value)||0), savings = Number((el('savings-goal')?.value)||0);
    const essentialsCap = Number((el('essentials-cap')?.value)||50), wantsCap = Number((el('wants-cap')?.value)||30), debtCap = Number((el('debt-cap')?.value)||20);
    ...
    store.monthlyBudget = essentials + debt; if (document.getElementById('monthly-budget')) document.getElementById('monthly-budget').value = store.monthlyBudget;
    prioritizeByBudget();
...
function renderTips() {
  const list = document.getElementById('tips-list'); if(!list) return;
  list.innerHTML = '';
  tipBank.forEach(t => { const div=document.createElement('div'); div.className='tip'; div.textContent=t; list.appendChild(div); });
}
...
if (otpBtn) otpBtn.addEventListener('click', () => { store.otpMonth = currentMonthStr(); const status = document.getElementById('otp-status'); if(status) status.textContent = `Active ${store.otpMonth}`; });
...
function renderAll(){ renderCalendar(); renderDashboard(); renderTips(); }
renderAll();

// Jump button from dashboard to calendar
document.querySelectorAll('[data-goto="calendar"]').forEach(btn => {
  btn.addEventListener('click', () => {
    document.querySelectorAll('.tab-btn').forEach(b => b.setAttribute('aria-selected', 'false'));
    document.querySelector('[data-tab="calendar"]').setAttribute('aria-selected','true');
    document.querySelectorAll('.tab').forEach(p => p.classList.remove('active'));
